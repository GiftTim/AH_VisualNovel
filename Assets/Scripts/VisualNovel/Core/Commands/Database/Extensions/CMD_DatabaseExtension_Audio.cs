...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_Audio : CMD_DatabaseExtension
    {
        ...
        private static void PlaySFX(string[] data)
        {
            ...
            if (sound == null)
            {
                Debug.LogError($"Could not load sound effect '{filepath}'. Please make sure this file exists in the Resources folder.");
                return;
            }
        ...
        private static void PlayVoice(string[] data)
        {
            ...
            //Run the logic
            AudioClip sound = Resources.Load<AudioClip>(
                FilePaths.GetPathToResource(FilePaths.resources_voices, filepath)
            );
