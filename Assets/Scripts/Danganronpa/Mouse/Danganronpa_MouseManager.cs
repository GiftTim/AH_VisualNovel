...
public class Danganronpa_MouseManager : MonoBehaviour
{
    [Header("Refs")]
    [SerializeField] Danganronpa_CameraController cameraController;
    [SerializeField] Danganronpa_KeyMover keyMover;
    private Danganronpa_MousePointer mousePointer;
    ...
    private void Awake()
    {
        mousePointer = new Danganronpa_MousePointer(tf_MousePointer);
    }
    ...
    private void UpdateDirectionArrows(Danganronpa_CameraController cv)
    {
        if (!cv) return;
