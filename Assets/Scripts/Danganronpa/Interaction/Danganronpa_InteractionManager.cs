...
public class Danganronpa_InteractionManager : MonoBehaviour
{
    [Header("Interaction에 관련된 스크립트")]
    [SerializeField] private Dangnaronpa_InteractionDetector interactionDetector;
    [SerializeField] private Danganronpa_InteractionUI interactionUI;

    [Header("참조하는 스크립트")]
    [SerializeField] private Dangnaronpa_DialogueManager dialogueManager;
    ...
    private void Awake()
    {
        if (!dialogueManager) dialogueManager = FindFirstObjectByType<Dangnaronpa_DialogueManager>();
    }
    ...
    private void FinishHoldAndExecute()
    {
        ...
        switch (activeTag)
        {
            case var t when t == tagNPC:
                ...
                if (dialogueManager && interactionDetector.HitInfo.transform.TryGetComponent(out Dangnaronpa_InteractionEvent ev))
                {

                }
                else
                {
                    Debug.Log("NPC 상호작용: Dangnaronpa_DialogueManager 또는 Dangnaronpa_InteractionEvent 없음");
